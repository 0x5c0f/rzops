...
impl AttachmentRepository for PgAttachmentRepository {
    async fn find_by_id(&self, id: Uuid) -> Result<Option<Attachment>, RepositoryError> {
        Ok(sqlx::query(&format!("SELECT {} FROM cmdb_attachment WHERE id=$1 AND deleted_at IS NULL", COLS)).bind(id).fetch_optional(&self.pool).await.repo()?.map(|r| row_to_entity(&r)))
    }
    async fn find_all(&self, f: AttachmentFilter) -> Result<Vec<Attachment>, RepositoryError> {
        let mut sql = format!("SELECT {} FROM cmdb_attachment WHERE deleted_at IS NULL", COLS);
    ...
    async fn count(&self, f: AttachmentFilter) -> Result<i64, RepositoryError> {
        let mut sql = "SELECT COUNT(*) as count FROM cmdb_attachment WHERE deleted_at IS NULL".to_string();
        let mut string_binds: Vec<String> = Vec::new();
        let mut uuid_binds: Vec<Uuid> = Vec::new();
        let mut idx = 1;
        if let Some(ref s) = f.status { sql.push_str(&format!(" AND status::text = ${}", idx)); string_binds.push(s.clone()); idx += 1; }
    ...
    async fn delete(&self, id: Uuid) -> Result<bool, RepositoryError> {
        Ok(sqlx::query("UPDATE cmdb_attachment SET deleted_at = now() WHERE id=$1 AND deleted_at IS NULL").bind(id).execute(&self.pool).await.repo()?.rows_affected() > 0)
    }
}
