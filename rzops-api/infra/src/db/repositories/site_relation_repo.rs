...
impl SiteServerRelationRepository for PgSiteRelationRepository {
    ...
    async fn find_sites_by_server(&self, server_id: Uuid) -> Result<Vec<SiteRefByServer>, RepositoryError> {
        let rows = sqlx::query(
            ...
            .bind(server_id).fetch_all(&self.pool).await.repo()?;
    ...
    async fn create(&self, e: &OpsSiteServer) -> Result<OpsSiteServer, RepositoryError> {
    ...
    async fn update(&self, id: Uuid, deploy_role: Option<String>) -> Result<bool, RepositoryError> {
        Ok(sqlx::query("UPDATE cmdb_ops_site_server SET deploy_role=$1 WHERE id=$2")
            .bind(deploy_role).bind(id).execute(&self.pool).await.repo()?.rows_affected() > 0)
    }
    async fn delete(&self, id: Uuid) -> Result<bool, RepositoryError> {
        Ok(sqlx::query("DELETE FROM cmdb_ops_site_server WHERE id=$1").bind(id).execute(&self.pool).await.repo()?.rows_affected() > 0)
    }
}
...
impl SiteDatabaseRelationRepository for PgSiteRelationRepository {
    ...
    async fn find_sites_by_database(&self, database_instance_id: Uuid) -> Result<Vec<SiteRefByDatabase>, RepositoryError> {
        let rows = sqlx::query(
            ...
            .bind(database_instance_id).fetch_all(&self.pool).await.repo()?;
    ...
    async fn create(&self, e: &OpsSiteDatabase) -> Result<OpsSiteDatabase, RepositoryError> {
    ...
    async fn delete(&self, id: Uuid) -> Result<bool, RepositoryError> {
        Ok(sqlx::query("DELETE FROM cmdb_ops_site_database WHERE id=$1").bind(id).execute(&self.pool).await.repo()?.rows_affected() > 0)
    }
}
...
impl SiteDomainRelationRepository for PgSiteRelationRepository {
    ...
    async fn delete(&self, id: Uuid) -> Result<bool, RepositoryError> {
        Ok(sqlx::query("DELETE FROM cmdb_ops_site_domain WHERE id=$1").bind(id).execute(&self.pool).await.repo()?.rows_affected() > 0)
    }
}
