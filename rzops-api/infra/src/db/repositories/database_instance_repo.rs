...
impl DatabaseInstanceRepository for PgDatabaseInstanceRepository {
    ...
    async fn find_all(&self, filter: DatabaseInstanceFilter) -> Result<Vec<DatabaseInstance>, RepositoryError> {
        ...
        sql.push_str(" ORDER BY CASE WHEN status::text IN ('retired', 'offline', 'inactive', 'disabled') THEN 2 WHEN server_id IS NULL OR server_id IN (SELECT id FROM cmdb_server WHERE deleted_at IS NOT NULL OR status::text IN ('retired', 'offline')) THEN 1 ELSE 0 END, created_at DESC");
