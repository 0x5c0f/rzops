...
impl DatabaseInstanceRepository for PgDatabaseInstanceRepository {
    ...
    async fn find_all(&self, filter: DatabaseInstanceFilter) -> Result<Vec<DatabaseInstance>, sqlx::Error> {
        ...
        sql.push_str(" ORDER BY CASE WHEN status::text IN ('retired', 'offline', 'inactive', 'disabled') THEN 2 WHEN server_id IS NOT NULL AND server_id IN (SELECT id FROM cmdb_server WHERE status::text IN ('retired', 'offline') AND deleted_at IS NULL) THEN 1 ELSE 0 END, created_at DESC");
