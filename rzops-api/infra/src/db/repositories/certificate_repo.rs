...
impl CertificateRepository for PgCertificateRepository {
    async fn find_by_id(&self, id: Uuid) -> Result<Option<Certificate>, RepositoryError> {
        let row = sqlx::query(
    ...
    async fn find_all(&self, filter: CertificateFilter) -> Result<Vec<Certificate>, RepositoryError> {
    ...
    async fn create(&self, c: &Certificate) -> Result<Certificate, RepositoryError> {
        let row = sqlx::query(
    ...
    async fn update(&self, id: Uuid, c: &Certificate) -> Result<Option<Certificate>, RepositoryError> {
        let row = sqlx::query(
    ...
    async fn delete(&self, id: Uuid) -> Result<bool, RepositoryError> {
        let result = sqlx::query("UPDATE cmdb_certificate SET deleted_at = now() WHERE id = $1 AND deleted_at IS NULL").bind(id).execute(&self.pool).await.repo()?;
