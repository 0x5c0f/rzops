...
async fn to_resp(ns: &Arc<dyn ResourceNameService>, e: &Attachment) -> AttachmentResponse {
    ...
    let uploader_name = match e.uploaded_by_id {
        Some(uid) => ns.resolve_user_name(uid).await,
        None => None,
    };
    AttachmentResponse {
        id: e.id,
...
pub async fn upload_attachment(auth: AuthUser, State(r): State<Arc<dyn AttachmentRepository>>, Extension(ns): Extension<Arc<dyn ResourceNameService>>, Extension(upload_dir): Extension<String>, Extension(change_log): Extension<ChangeLogState>, mp: Multipart) -> impl IntoResponse {
    ...
    match r.create(&e).await {
        Ok(c) => {
            record_change(&change_log, &auth, rzops_domain::enums::ChangeType::Create, "attachment", Some(c.id), serde_json::json!(null), serde_json::to_value(to_resp(&ns, &c).await).unwrap_or(serde_json::json!({})), None).await;
...
pub async fn get_attachment(_auth:AuthUser,State(r):State<Arc<dyn AttachmentRepository>>,Extension(ns):Extension<Arc<dyn ResourceNameService>>,Path(id):Path<Uuid>)->impl IntoResponse{match r.find_by_id(id).await{Ok(Some(e))=>(StatusCode::OK,Json(to_resp(&ns,&e).await)).into_response(),Ok(None)=>(StatusCode::NOT_FOUND,Json(ErrorResponse{error:"not found".into()})).into_response(),Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(ErrorResponse{error:e.to_string()})).into_response()}}
...
pub async fn create_attachment(auth:AuthUser,State(r):State<Arc<dyn AttachmentRepository>>,Extension(ns):Extension<Arc<dyn ResourceNameService>>,Extension(change_log):Extension<ChangeLogState>,Json(b):Json<CreateAttachmentRequest>)->impl IntoResponse{
...
pub async fn update_attachment(auth:AuthUser,State(r):State<Arc<dyn AttachmentRepository>>,Extension(ns):Extension<Arc<dyn ResourceNameService>>,Extension(change_log):Extension<ChangeLogState>,Path(id):Path<Uuid>,Json(b):Json<CreateAttachmentRequest>)->impl IntoResponse{
    ...
    match r.update(id,&e).await{
        Ok(Some(c))=>{
            record_change(&change_log,&auth,rzops_domain::enums::ChangeType::Update,"attachment",Some(c.id),before_value,serde_json::to_value(to_resp(&ns,&c).await).unwrap_or(serde_json::json!({})),None).await;
