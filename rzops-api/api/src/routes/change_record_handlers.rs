...
fn ct_to_string(t: &ChangeType) -> String {
    match t {
...
fn to_resp(e: &rzops_domain::models::change_record::ChangeRecord, actor_email: Option<String>, resource_name: Option<String>) -> ChangeRecordResponse {
    ChangeRecordResponse {
        id: e.id,
        actor_id: e.actor_id,
        change_type: ct_to_string(&e.change_type),
        resource_type: e.resource_type.clone(),
        resource_id: e.resource_id,
        actor_email,
        resource_name,
        before_data: e.before_data.clone(),
        after_data: e.after_data.clone(),
...
pub async fn get_change_record(
    auth: AuthUser,
    State(r): State<Arc<dyn ChangeRecordRepository>>,
...
pub async fn list_change_records(
    auth: AuthUser,
    State(r): State<Arc<dyn ChangeRecordRepository>>,
    Extension(pool): Extension<PgPool>,
    Query(q): Query<ListChangeRecordsQuery>,
) -> impl IntoResponse {
    if let Err(resp) = auth.require_superuser() { return resp }
    ...
    match r.find_all(f.clone()).await {
        Ok(v) => {
            ...
            (StatusCode::OK, Json(ChangeRecordListResponse { data, count: c })).into_response()
        }
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(ErrorResponse { error: e.to_string() })).into_response(),
    }
}
