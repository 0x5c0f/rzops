...
pub async fn create_database_instance(auth: AuthUser, State(repo): State<Arc<dyn DatabaseInstanceRepository>>, Extension(change_log): Extension<ChangeLogState>, Json(body): Json<CreateDatabaseInstanceRequest>) -> impl IntoResponse {
    let now = Utc::now();
    // 提取字段：name 与 instance_name 都可能由前端单独/同时提交，先取值避免部分 move
    let body_name = body.name;
    let body_instance_name = body.instance_name;
    let d = DatabaseInstance { id: Uuid::new_v4(), server_id: body.server_id, name: body_name.clone(), db_type: body.db_type, description: body.description, status: body.status.unwrap_or_else(|| "active".to_string()), environment: body.environment, offline_time: body.offline_time, is_self_installed: body.is_self_installed.unwrap_or(true), importance: body.importance, is_ops_managed: body.is_ops_managed.unwrap_or(true), port: body.port, instance_name: body_instance_name.or(Some(body_name)), created_at: now, updated_at: now, deleted_at: None };
...
pub async fn update_database_instance(auth: AuthUser, State(repo): State<Arc<dyn DatabaseInstanceRepository>>, Extension(change_log): Extension<ChangeLogState>, Path(id): Path<Uuid>, Json(body): Json<UpdateDatabaseInstanceRequest>) -> impl IntoResponse {
    ...
    // 提取字段以避免部分 move 后无法继续访问 body.name（instance_name 与 name 均可能由前端单独/同时提交）
    let final_name = body.name.clone().unwrap_or(existing.name.clone());
    let final_instance_name = body.instance_name.clone().or(existing.instance_name.clone()).or(Some(final_name.clone()));
    let final_status = body.status.as_deref().map(str::trim).filter(|s| !s.is_empty()).map(ToString::to_string).unwrap_or_else(|| existing.status.clone());
    let d = DatabaseInstance { id: existing.id, server_id: body.server_id.or(existing.server_id), name: final_name, db_type: body.db_type.unwrap_or(existing.db_type), description: body.description.or(existing.description), status: final_status, environment: body.environment.or(existing.environment), offline_time: body.offline_time.or(existing.offline_time), is_self_installed: body.is_self_installed.unwrap_or(existing.is_self_installed), importance: body.importance.or(existing.importance), is_ops_managed: body.is_ops_managed.unwrap_or(existing.is_ops_managed), port: body.port.or(existing.port), instance_name: final_instance_name, created_at: existing.created_at, updated_at: Utc::now(), deleted_at: existing.deleted_at };
