...
/// 根据备份目标类型与目标 ID 解析资源名称（表名列名来自固定白名单，非用户输入）
...
async fn to_resp(pool: &PgPool, e: &BackupPlan) -> BackupPlanResponse {
...
pub async fn get_backup_plan(_auth: AuthUser, State(r): State<Arc<dyn BackupPlanRepository>>, Extension(pool): Extension<PgPool>, Path(id): Path<Uuid>) -> impl IntoResponse {
    match r.find_by_id(id).await{Ok(Some(e))=>(StatusCode::OK,Json(to_resp(&pool,&e).await)).into_response(),Ok(None)=>(StatusCode::NOT_FOUND,Json(ErrorResponse{error:"not found".into()})).into_response(),Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(ErrorResponse{error:e.to_string()})).into_response()}
}
...
pub async fn list_backup_plans(_auth: AuthUser, State(r): State<Arc<dyn BackupPlanRepository>>, Extension(pool): Extension<PgPool>, Query(q): Query<ListBackupPlansQuery>) -> impl IntoResponse {
...
pub async fn create_backup_plan(auth: AuthUser, State(r): State<Arc<dyn BackupPlanRepository>>, Extension(pool): Extension<PgPool>, Extension(change_log): Extension<ChangeLogState>, Json(b): Json<CreateBackupPlanRequest>) -> impl IntoResponse {
    ...
    match r.create(&e).await{
        Ok(c)=>{
            record_change(&change_log,&auth,rzops_domain::enums::ChangeType::Create,"backup_plan",Some(c.id),serde_json::json!(null),serde_json::to_value(to_resp(&pool,&c).await).unwrap_or(serde_json::json!({})),None).await;
...
pub async fn update_backup_plan(auth: AuthUser, State(r): State<Arc<dyn BackupPlanRepository>>, Extension(pool): Extension<PgPool>, Extension(change_log): Extension<ChangeLogState>, Path(id): Path<Uuid>, Json(b): Json<UpdateBackupPlanRequest>) -> impl IntoResponse {
    ...
    match r.update(id,&e).await{
        Ok(Some(c))=>{
            record_change(&change_log,&auth,rzops_domain::enums::ChangeType::Update,"backup_plan",Some(c.id),before_value,serde_json::to_value(to_resp(&pool,&c).await).unwrap_or(serde_json::json!({})),None).await;
