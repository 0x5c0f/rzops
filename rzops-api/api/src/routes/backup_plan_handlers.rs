...
fn to_resp(e: &BackupPlan) -> BackupPlanResponse {
    BackupPlanResponse { id:e.id,name:e.name.clone(),target_type:e.target_type.clone(),target_id:e.target_id,schedule:e.schedule.clone(),retention_days:e.retention_days,status:e.status.clone(),remarks:e.remarks.clone(),created_at:e.created_at,updated_at:e.updated_at }
}
...
pub async fn create_backup_plan(auth: AuthUser, State(r): State<Arc<dyn BackupPlanRepository>>, Extension(change_log): Extension<ChangeLogState>, Json(b): Json<CreateBackupPlanRequest>) -> impl IntoResponse {
    ...
    match r.create(&e).await{
        Ok(c)=>{
            record_change(&change_log,&auth,rzops_domain::enums::ChangeType::Create,"backup_plan",Some(c.id),serde_json::json!(null),serde_json::to_value(to_resp(&c)).unwrap_or(serde_json::json!({})),None).await;
...
pub async fn update_backup_plan(auth: AuthUser, State(r): State<Arc<dyn BackupPlanRepository>>, Extension(change_log): Extension<ChangeLogState>, Path(id): Path<Uuid>, Json(b): Json<UpdateBackupPlanRequest>) -> impl IntoResponse {
    ...
    match r.update(id,&e).await{
        Ok(Some(c))=>{
            record_change(&change_log,&auth,rzops_domain::enums::ChangeType::Update,"backup_plan",Some(c.id),before_value,serde_json::to_value(to_resp(&c)).unwrap_or(serde_json::json!({})),None).await;
