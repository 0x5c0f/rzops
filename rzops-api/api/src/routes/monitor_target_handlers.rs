...
fn to_resp(e:&MonitorTarget)->MonitorTargetResponse{MonitorTargetResponse{id:e.id,name:e.name.clone(),site_id:e.site_id,target_type:e.target_type.clone(),target_id:e.target_id,monitor_type:e.monitor_type.clone(),endpoint:e.endpoint.clone(),interval_seconds:e.interval_seconds,status:e.status.clone(),remarks:e.remarks.clone(),created_at:e.created_at,updated_at:e.updated_at}}
...
pub async fn create_monitor_target(auth: AuthUser, State(r):State<Arc<dyn MonitorTargetRepository>>, Extension(change_log): Extension<ChangeLogState>, Json(b):Json<CreateMonitorTargetRequest>)->impl IntoResponse{
    let now=Utc::now();let e=MonitorTarget{id:Uuid::new_v4(),name:b.name,site_id:b.site_id,target_type:b.target_type,target_id:b.target_id,monitor_type:b.monitor_type,endpoint:b.endpoint,interval_seconds:b.interval_seconds,status:b.status.unwrap_or_else(|| "draft".to_string()),remarks:b.remarks,created_at:now,updated_at:now};
