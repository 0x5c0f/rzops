...
fn to_response(d: &DomainAsset) -> DomainResponse {
    DomainResponse { id: d.id, domain_name: d.domain_name.clone(), registered_date: d.registered_date, expiry_date: d.expiry_date, renewal_amount: d.renewal_amount.map(|v| v.to_string()), renewal_currency: d.renewal_currency.clone(), provider_id: d.provider_id, platform_phone: d.platform_phone.clone(), domain_email: d.domain_email.clone(), privacy_status: d.privacy_status.clone(), is_enabled: d.is_enabled, remarks: d.remarks.clone(), created_at: d.created_at, updated_at: d.updated_at, deleted_at: d.deleted_at }
}
fn build_from_create(b: CreateDomainRequest) -> DomainAsset {
    let now = Utc::now();
    DomainAsset { id: Uuid::new_v4(), domain_name: b.domain_name, registered_date: b.registered_date, expiry_date: b.expiry_date, renewal_amount: b.renewal_amount.and_then(|v| v.parse::<Decimal>().ok()), renewal_currency: b.renewal_currency.unwrap_or_else(|| "CNY".to_string()), provider_id: b.provider_id, platform_phone: b.platform_phone, domain_email: b.domain_email, privacy_status: b.privacy_status, is_enabled: b.is_enabled.unwrap_or(true), remarks: b.remarks, created_at: now, updated_at: now, deleted_at: None }
}
...
pub async fn update_domain(auth: AuthUser, State(repo): State<Arc<dyn DomainRepository>>, Extension(change_log): Extension<ChangeLogState>, Path(id): Path<Uuid>, Json(body): Json<UpdateDomainRequest>) -> impl IntoResponse {
    ...
    let d = DomainAsset { id: existing.id, domain_name: body.domain_name.unwrap_or(existing.domain_name), registered_date: body.registered_date.or(existing.registered_date), expiry_date: body.expiry_date.or(existing.expiry_date), renewal_amount: body.renewal_amount.and_then(|v| v.parse::<Decimal>().ok()).or(existing.renewal_amount), renewal_currency: body.renewal_currency.unwrap_or(existing.renewal_currency), provider_id: body.provider_id.or(existing.provider_id), platform_phone: body.platform_phone.or(existing.platform_phone), domain_email: body.domain_email.or(existing.domain_email), privacy_status: body.privacy_status.or(existing.privacy_status), is_enabled: body.is_enabled.unwrap_or(existing.is_enabled), remarks: body.remarks.or(existing.remarks), created_at: existing.created_at, updated_at: Utc::now(), deleted_at: existing.deleted_at };
