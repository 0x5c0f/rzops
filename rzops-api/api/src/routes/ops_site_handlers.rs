...
fn to_response(s: &OpsSite) -> OpsSiteResponse {
    OpsSiteResponse { id: s.id, name: s.name.clone(), url: s.url.clone(), business_unit_id: s.business_unit_id, department_id: s.department_id, service_target: s.service_target.clone(), importance: s.importance.clone(), online_time: s.online_time, code_repo_type: s.code_repo_type.clone(), code_repo_url: s.code_repo_url.clone(), purpose: s.purpose.clone(), is_internal_system: s.is_internal_system, language_runtime: s.language_runtime.clone(), web_framework: s.web_framework.clone(), uses_cdn: s.uses_cdn, is_test_site: s.is_test_site, backup_plan_id: s.backup_plan_id, last_backup_time: s.last_backup_time, monitor_target_id: s.monitor_target_id, status: s.status.clone(), offline_time: s.offline_time, offline_reason: s.offline_reason.clone(), function_summary: s.function_summary.clone(), remarks: s.remarks.clone(), created_at: s.created_at, updated_at: s.updated_at }
}
...
pub async fn create_ops_site(auth: AuthUser, State(repo): State<Arc<dyn OpsSiteRepository>>, Extension(change_log): Extension<ChangeLogState>, Json(body): Json<CreateOpsSiteRequest>) -> impl IntoResponse {
    let now = Utc::now();
    let s = OpsSite { id: Uuid::new_v4(), name: body.name, url: body.url, business_unit_id: body.business_unit_id, department_id: body.department_id, service_target: body.service_target, importance: body.importance, online_time: body.online_time, code_repo_type: body.code_repo_type, code_repo_url: body.code_repo_url, purpose: body.purpose, is_internal_system: body.is_internal_system.unwrap_or(false), language_runtime: body.language_runtime, web_framework: body.web_framework, uses_cdn: body.uses_cdn, is_test_site: body.is_test_site.unwrap_or(false), backup_plan_id: body.backup_plan_id, last_backup_time: body.last_backup_time, monitor_target_id: body.monitor_target_id, status: body.status.unwrap_or_else(|| "active".to_string()), offline_time: body.offline_time, offline_reason: body.offline_reason, function_summary: body.function_summary, remarks: body.remarks, created_at: now, updated_at: now };
