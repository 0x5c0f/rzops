...
- **Regression**: all 8 BUGs verified through the browser (see UI_TEST_REPORT §六); recycle bin 502 also re-verified OK. **Manual recheck needed**: under bu automation the port-new server dialog does not close after clicking "确认" (form IS backfilled; Esc/Close/overlay all fail — suspected bits-ui Dialog + portal + automation compatibility; the feature itself works — the multi-select create succeeded).
