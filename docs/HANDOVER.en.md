...
- **Provider type "cannot unselect" interaction fix**: In `FormMultiSelect`, previously-selected items in the dropdown were `disabled` (clicking them did nothing; removal only worked via the × on the trigger badge, which users did not discover). Changed to **click-to-toggle**: clicking a selected item unselects it, clicking an unselected item selects it; selected items get `text-primary` highlight plus a "已选（点击取消）" hint; also added `onkeydown` (Enter/Space) on the × to clear the a11y warning. **RESOLVED**: the user confirmed multi-select IS the product design for provider types (one provider can be marked with several types) — do NOT switch to single-select; see Round 13 for the follow-up interaction fixes to FormMultiSelect.
...
### 2026-09-15 Round 6: Real root cause of duplicate-IP reminder (missing /api/v1 prefix) + unified unlinked-target labels
- **Root cause of "reminder still not working" (major pitfall)**: After rebuilding the container and confirming the check request fired (with `_t`, HTTP 200), the console showed `Duplicate IP check failed: SyntaxError: Unexpected token '<', "<!doctype "... is not valid JSON`. **Cause: the frontend call `api.get('/server-ips', ...)` omitted the `/api/v1` prefix** — `client.ts` `request()` does NOT auto-prepend a base URL; nginx serves the SPA fallback (`index.html`) for non-`/api/` paths, `JSON.parse` fails, the catch silently skips, no dialog. **Fix**: path corrected to `/api/v1/server-ips`; verified via curl (`/api/v1/server-ips` → JSON, `/server-ips` → `<!doctype html>`).
- **Same round**: added `_t=Date.now()` cache-bust to the check request (browser memory-cache can reuse the same-URL GET response, which caused the second add to skip); added `console.info('[dup-check] ...')` observability log.
- **Convention (important)**: all `api.get/post/put/delete` calls in this frontend MUST include the full `/api/v1/...` path; missing the prefix does not error loudly — nginx returns HTML and the call fails silently.
- **Unified "unlinked target" labels**: list/detail pages for the "关联目标/所属服务器/服务器" columns now show **"未关联目标"** (target-type) / **"未关联服务器"** (server-ownership) instead of `-`; "已删除" unified to **"服务器已删除"** (server-ips / database-instances / server-ports detail). Row-level color logic fully preserved (deleted → text-red-500, retired → text-amber-600). Files: backup-plans, monitor-targets, server-ips, database-instances (list+detail), server-ports detail (9 spots).
- **Todo (evaluation given, pending decision)**: deletion semantics of associated cards on server/site/database-instance edit pages — `ServerForm.syncIps`/`syncDbInstances` and `OpsSiteForm`/`DatabaseInstanceForm` syncBackupPlans/syncMonitorTargets currently soft-delete the child record. Proposal: unbind instead (IP/DB `server_id` → null; backup-plan/monitor-target `target_type/target_id` → null); ports (per-server records, `UNIQUE(server_id,protocol,port)`) deleting own record is fine; site-relations / certificate-domains (junction tables) already correct. Backend needs 4 new unbind endpoints.

### 2026-09-15 Round 7: Edit-page "remove association" now unbinds (4 new backend endpoints)
- **Background**: user found that deleting an added IP on the server edit page actually soft-deleted the IP record itself (`serverIpsApi.delete`) instead of unbinding it from the server — IP/database-instance records are independent assets with their own management pages, so removing an association should keep the record reusable. Evaluated 10 spots: 6 changed to unbind (ServerForm.syncIps/syncDbInstances, OpsSiteForm.syncBackupPlans/syncMonitorTargets, DatabaseInstanceForm same), 4 already correct (ports = per-server records `UNIQUE(server_id,protocol,port)`; site-relations / certificate-domains = junction tables).
- **Backend (4 unbind endpoints)**: `POST /api/v1/{entity}/{id}/unbind` — server-ips/database-instances set `server_id=NULL`, backup-plans/monitor-targets set `target_type=NULL,target_id=NULL`; `WHERE id=$1 AND deleted_at IS NULL RETURNING ...`; each records a `ChangeType::Update` change log. Touched: 4 domain `ports/*_repository.rs` traits (+`unbind`), 4 infra `repositories/*_repo.rs`, 4 `routes/*_handlers.rs`, `routes/mod.rs` (`.route("/{id}/unbind", post(...))`), `openapi.rs`. Pitfall: monitor_target delete handler signature is `State(r):State<...>` (no space) — Edit exact-match failed; match the original text.
- **Frontend**: 4 `lib/api/*.ts` added `unbind: (id) => api.post(`/api/v1/.../${id}/unbind`)`; 3 forms switched `Api.delete` → `Api.unbind` in their sync functions; ports/site-relations/certificate-domains untouched.
- **Test results**: backend curl e2e all green (login → create 4 bound records → unbind each HTTP 200 → GET shows server_id/target_id null and record still present); `cargo check` + `cargo build --release --target x86_64-unknown-linux-musl` (1m44s) no warnings; `npm run build` no warnings; after `--no-cache` container rebuild, grep confirmed `.unbind(` calls and the 4 API paths are inside chunks (ServerForm sync now looks like `some(t=>t.id===e.id)&&await R.unbind(e.id)`).
- **Pitfall (commit hygiene)**: `git add -A` also staged workspace `tmp_*.sh` scripts — check `git status --short` before adding, or keep scratch scripts in a subdir; clean with `git rm <file>` + separate commit.
- **bu sandbox status**: login click still produces no network request (fetch proxy broken) — UI automation unavailable; this round's frontend verification relies on backend curl e2e + frontend build + container chunk grep + user manual re-check.

### 2026-09-16 Round 8: F5 mis-logout fix + IP unbound-state color/filter + server-column link + recycle JSON view
- **Issue 1 (F5 mis-logout) root cause & fix**: `+layout.svelte` onMount calls `authApi.me()`; pressing F5 aborts that in-flight fetch (`DOMException: AbortError`), and the catch block misjudged it as auth failure → `auth.logout()` cleared the token → next page load had no token → redirected to login. **Fix**: in catch, first check `err instanceof DOMException && err.name === 'AbortError'` and return without logout/redirect. client.ts only clears the token on an actual 401 response; aborts never reach that path, so no change needed there.
- **Issue 2 (unbound state)**:
  - Color: server-ips `getRowClass` added an "未关联服务器" branch → `text-sky-600` (blue; distinct from disabled-gray text-slate-400, deleted-red text-red-500, retired-amber text-amber-600); backup-plans / monitor-targets also got "未关联目标" → `text-sky-600`; database-instances keeps its existing "未选择服务器" amber.
  - Filter: backend `ServerIpFilter` + `ListServerIpsQuery` dto added `server_bound: Option<bool>`; repo `find_all` **and `count`** both apply `AND server_id IS (NOT) NULL` (**pitfall: first pass only patched find_all; count kept returning the total, so filtering showed correct rows but wrong total**); frontend advanced filter added a "关联状态" dropdown (全部/已关联/未关联) + active-filter chip. curl verified: bound count=2, unbound count=5, total 7.
- **Issue 3 (server column link)**: server-ips list server column added `link: (item) => item.server_id ? `/servers/${item.server_id}` : null` (DataTable's per-item link support; null renders plain text, so unbound/deleted rows are not clickable).
- **Issue 4 (recycle JSON view)**: expanded detail switched from key-value grid to a formatted JSON view (`<pre>` + `JSON.stringify(item.data, null, 2)` + muted background + max-height scroll); removed now-unused HIDDEN_FIELDS / formatSnapshotValue.
- **Tests**: cargo check / npm run build clean; after `--no-cache` container rebuild, grep confirmed server_bound, text-sky-600, server-detail links, JSON.stringify snapshot, and AbortError are all inside the built chunks; backend curl verified server_bound=true/false rows and counts.
- **bu sandbox still broken** (login click produces no network request); frontend interaction verification relies on container chunk grep + backend curl + user manual re-check.

### 2026-09-16 Round 9: IP edit rebind/unbind closure + color design (text-level) + Playwright front-end E2E
- **IP rebind/unbind closure**: the IP edit page previously stripped `server_id` in `handleUpdate` (backend `UpdateServerIpRequest` had no such field), so an IP unbound from the server edit page could never be re-bound (only deletable). **Fix**: backend dto added `server_id: Option<Uuid>` (uuid=rebind, absent=unchanged); repo update SQL added `server_id = $10` + bind; frontend ServerIpForm removed `disabled={editing}`; edit page handleUpdate branches: `server_id === '' && was bound` → `update(rest)` then `unbind(id)` (clear goes through the dedicated endpoint); otherwise `update({...rest, server_id})` (rebind). **Verified**: edit an unbound IP → pick test001 → save → detail shows test001 link; clear selection → save → detail shows "未关联服务器".
- **serde pitfall (documented)**: `Option<Option<Uuid>>` cannot distinguish "field absent" from "field null" — serde deserializes JSON null for any `Option<T>` into outer `None` (not `Some(None)`). So clearing a binding cannot be done via update with null; it must go through a dedicated endpoint (e.g. POST /{id}/unbind). Round first implemented Option<Option<Uuid>> + unwrap_or; after testing showed null didn't clear, switched to Option<Uuid> + unbind endpoint.
- **Color design change (explicit user intent)**: relation states like "未关联服务器" should only get **text-level coloring** (that cell's text only), not whole-row coloring; whole-row color follows the entity's own main status only. **Implementation**: DataTable gained column prop `cellClass?: (item: T) => string | null` (applied per cell: `cn('truncate', col.class, col.cellClass?.(item) ?? '')`); four lists adjusted — server-ips / database-instances getRowClass slimmed to main-status only (disabled gray), server column got cellClass (unbound→text-purple-600, deleted→text-red-500, retired→text-amber-600); backup-plans / monitor-targets likewise (their target column key is `target_name`, not `target_id` — mind this when regex-matching column defs). **Pitfall**: BP/MT pages had not imported `isResourceOffline`; cellClass introduced a runtime ReferenceError (frontend build passes, breaks at runtime) — must add the import.
- **Playwright MCP connector enabled**: user installed playwright-browser-automation MCP, enabling real front-end E2E (bu sandbox was broken). Used it this round: login → 3× F5 keeps token without redirect; server-ips unbound rows text-purple only; advanced filter "关联状态" dropdown (unbound 3 / bound 4 + chips); server-column click navigates to `/servers/{id}`; edit-page rebind/unbind closure; recycle expanded JSON view (`<pre>` + mono font + max-height 320px scroll); backup-plans / monitor-targets / database-instances text coloring. Tools are `mcp__playwright__browser_*`: browser_navigate → browser_snapshot for refs → browser_click/browser_type/browser_select_option (needs values array)/browser_run_code_unsafe (page-level JS assertions).

### 2026-09-16 Round 10: Collapsed sidebar menu supports click-to-popover submenu for direct navigation
- **Problem**: in collapsed mode menu items are icons only; the submenu was a pure-CSS `group-hover` flyout — unusable for non-hover/touch users, forcing them to expand the sidebar to pick an item.
- **Approach** (following the collapsed-menu convention of Ant Design Pro / Element Plus etc.): hover preview + **click the icon to pin an open Popover submenu**, click a child navigates immediately and closes, clicking anywhere else closes. Desktop hover previews, touch clicks trigger — both coexist.
- **Implementation**: Sidebar.svelte added `collapsedPopover` (group pinned by click) and `hoverGroup` (group under hover); visibility = `collapsedPopover === label || hoverGroup === label`; button onclick `stopPropagation` then toggles collapsedPopover; child onclick `stopPropagation` + clears both states + onMobileClose; a `$effect` listens for window click and closes when collapsedPopover is set (click-outside). Flyout got `role="menu"`, button got `aria-expanded/aria-haspopup`.
- **Verified** (Playwright MCP): collapse → click "网络" icon → menu flyout shows 5 children → click "服务器IP" → URL becomes /server-ips, flyout closes, sidebar stays 64px collapsed.

### 2026-09-16 Round 10-fix: Two root causes for "collapsed popover not visible" (cache + occlusion)
- **Symptom**: after Round 10, manual testing showed no popover in collapsed mode, plus console resource 404 / errors.
- **Root cause A (nginx caching)**: rzops-web/nginx.conf SPA fallback `location /` had only `try_files`, **no cache headers** → browsers heuristically cached index.html → after deploy, old HTML referenced old-hash assets already removed by the new build → **JS 404 / feature not working**. **Fix**: `location /` gained `add_header Cache-Control "no-cache, must-revalidate"` (HTML revalidates every time); `/_app/` assets keep `expires 30d; Cache-Control public, immutable` (hashed assets cached forever — correct). **Verified**: `curl -sI /` → `no-cache, must-revalidate`; assets → `public, immutable` + `max-age=2592000`.
- **Root cause B (popover occluded, important)**: in collapsed mode the aside is `md:static` (**position: static makes z-index useless**), nav has `overflow-x-hidden`, and the `absolute left-full` popover overflows nav width and is **clipped**, while main content (later in DOM order) covers it — **computed visibility is visible, but elementFromPoint at the popover center hits the list header instead → invisible on screen**. **Fix**: collapsed popover changed to `fixed` + `z-[9999]`, anchored from the button's `getBoundingClientRect()` (`top: r.top; left: r.left + r.width + 8`) stored in `popoverAnchor` state, refreshed on mouseenter/click; `$effect` closes on window click + scroll (capture=true to catch nav inner scroll). **Verified**: elementFromPoint hits the popover itself (inMenu=true), position=fixed, z-index=9999.
- **MCP Playwright screenshot pitfall**: `browser_take_screenshot` / `page.screenshot()` inside run_code_unsafe can be out of sync with actual page state (connector reloads/resets page context between calls, collapsed state lost → screenshots show expanded mode) — **DOM-level assertions (evaluate + elementFromPoint) are the reliable verification; screenshots are reference only**.
- **"Build error" clarification**: both `npm run build` and `docker compose build web` complete outputs show zero errors/warnings (checked; `[PLUGIN_TIMINGS]` is just a performance notice); the console errors the user saw were the root-cause-A stale-asset 404 chain.

### 2026-09-16 Round 11: Collapsed popover parent/child menu hierarchy
- Need: in collapsed mode, the popover showed the group label (e.g. "网络") and its items (域名/证书/服务器IP/...) all in one flat column with no hierarchy.
- Implementation (Sidebar.svelte collapsed branch): group header is now `flex items-center gap-2 border-b border-sidebar-border px-2 pb-2 pt-1` (group icon + semibold name + 1px bottom divider); sub-item container `ml-3 ... border-l border-sidebar-border pl-2 pt-1` (12px indent + 1px guide line, matching expanded-mode style); active sub-item gets `absolute -left-[11px] ... rounded-full bg-sidebar-primary` bar indicator (consistent with expanded highlight). Popover width min-w-44 → min-w-48.
- Verified: DOM asserts headerText=网络, headerHasBorder=true, 5 sub-items all indented; elementFromPoint hits the popover (still fixed z-[9999]). The MCP screenshot vs page-state desync issue persists (connector reloads page context per call) — DOM assertions are authoritative.
- Note: the group header is not clickable (no standalone page), visual hierarchy only.

### 2026-09-16 Round 12: Rapid-F5 logout regression fixed (only 401/403 clears session)
- **Symptom**: pressing F5 very rapidly logs the user out. nginx evidence: `GET /api/v1/auth/me` returned **200** (token valid) yet the frontend redirected to `/login` — **frontend misjudged a non-401 transient error as auth failure**.
- **Root cause**: Round 8 only handled `DOMException AbortError` in the catch; on rapid refresh the unloading page cancels in-flight fetches and the browser throws **`TypeError: Failed to fetch`** (not AbortError) → fell into `auth.logout() + goto('/login')` → logged out.
- **Fix** (+layout.svelte onMount catch): **only a clear `ApiError` with status 401/403 clears the session and redirects**; `AbortError` / `TypeError` (page-unload cancellation / transient network failure) / other non-auth errors all **keep the session** (just `loading = false` to end the loading state). Added `import { ApiError } from '$lib/api/client'`.
- **Verified**: Playwright logged in then did 8 rapid reloads (100ms apart) — still on `/`, `localStorage.token` present, no redirect to `/login`.
- **Design principle**: auth-check error handling should only take a logout action on an explicit 401/403; network interruption or transient failures must not kill the session — better to skip refreshing the user this round than to wrongly log out.

### 2026-09-16 Round 13: FormMultiSelect triple interaction fixes (bits-ui deep gotchas)

All three bugs live in `rzops-web/src/lib/components/shared/FormMultiSelect.svelte`, used by 4 fields: provider type, server role tags, server web-server software, datacenter line types. Verified end-to-end with real-mouse Playwright clicks on both 8081 (e2e) and 8080 (user env).

- **① ×-remove button dead on real click**: the remove `<span role=button>` on each selected Badge only had `onclick`; **real mouse clicks did nothing** while programmatic `el.click()` worked. Root cause: bits-ui `Select.Trigger` opens its popover **during pointerdown**, the popover covers the click position and swallows the subsequent `click` — the Badge's `onclick` never fires. Fix: add `onpointerdown` (`stopPropagation + preventDefault + removeValue`) on the remove span; keep `onclick`/`onkeydown` as fallbacks.
- **② One click deletes multiple values ("too sensitive")**: after pointerdown removes a Badge the DOM reflows immediately — the next Badge's × slides left into the mouse position, and the browser-synthesized `click` lands on it and removes it too. Fix: record a timestamp on pointerdown/keydown removal and **ignore any `click` within 350ms** (`REMOVE_SUPPRESS_MS`); `onclick` remains only as a fallback when pointerdown never fired.
- **③ Cannot unselect when only one value is selected / first option misbehaves when empty**: the component used `type="single" value=""` with manual toggle inside `onValueChange`. bits-ui Select is **semi-controlled**: after the first select its internal state remembers the value (a constant `value=""` prop is not re-synced back), so clicking the already-selected option fires the **cancel callback `onValueChange("")`**, which the `if (v)` guard silently dropped. Fix: switch to **`type="multiple"` with `value={currentValue}` and `onValueChange={(v) => value = v}`** — bits-ui natively toggles items and returns the full new array (no empty-string cancel callback); the "已选（点击取消）" marker is rendered from `currentValue`, unaffected. Deleted the now-unused `toggleValue`/`addValue`.
- **Gotchas (generalize)**: ① any interactive element embedded inside a bits-ui Select.Trigger must intercept at **pointerdown**, never rely on click alone; ② when a pointerdown handler removes DOM, suppress the following synthesized click (timestamp window) or the click hits a reflowed neighbor; ③ to build a multi-select with bits-ui, prefer **`type="multiple"`** over `single` + manual toggle — the single-select cancel callback passes an empty string that is easy to drop; ④ bits-ui dropdown open and option selection only respond to **real pointer events** — programmatic `el.click()` does not work, so tests must use real mouse events (Playwright `page.mouse.click`).
- **Related sweep (no other issues)**: all other bits-ui Select usages are safe — `FormSelect` and the list-page filters (audit-logs/change-records/dicts/recycle) use `bind:value` (full controlled loop, cancel syncs an empty value back) plus an explicit `<Select.Item value="">全部</Select.Item>`; `RemoteSearchSelect`/`TableSelectModal` are hand-rolled (no bits-ui), their toggle logic is self-implemented and correct.
