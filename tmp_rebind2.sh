...
docker compose up -d api web 2>&1 | tail -2
sleep 8
...
echo '=== 3) 清空（POST unbind）==='
curl -s -X POST "$API/server-ips/$IPID/unbind" -H "$AUTH" | python3 -c "import sys,json;d=json.load(sys.stdin);print('server_id=',d.get('server_id'))"
...
echo '=== 6) 容器产物验证 ==='
echo -n 'purple-600: '
docker exec rzops-web-1 sh -c "grep -rl 'text-purple-600' /usr/share/nginx/html/_app/immutable/ | wc -l"
echo -n 'server_id 改绑逻辑: '
docker exec rzops-web-1 sh -c "grep -rl 'unbind' /usr/share/nginx/html/_app/immutable/nodes/ | wc -l"
